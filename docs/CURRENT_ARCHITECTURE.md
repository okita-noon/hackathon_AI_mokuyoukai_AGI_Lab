...
Stripe未設定では罰金は `MOCKED` の記録です。設定時は `REQUIRES_ACTION` の請求を作り、Stripe Checkoutで支払いを受け付けます。テストキー専用のテスト決済、画面のポーリングと支払い後の確認ページを備えます。旧バックエンドの自動課金処理と混同しないでください。本格公開に向けては、認証・アカウント復旧、利用量制限、保存期間と削除、AI費用の管理、実課金時の判定・異議申し立て・送金先検証を別途設計する必要があります。
