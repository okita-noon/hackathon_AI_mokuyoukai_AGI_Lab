...
1. **自分を知ってもらう。** Gmail・X・LINEを題材にしたサンプル履歴と、任意の補足をもとに、AIが行動パターンを読み取ります。
...
| 対象 | 内容 |
|---|---|
| このリポジトリの公開デモ | サンプル履歴による見立て、約束、写真・動画の判定、未達時の叱咤とモック罰金の記録 |
| 別途開発された完成版のサービス紹介 | 自分のSNS・メール・AI会話履歴を渡した個別理解と、赤十字などの団体・友人・母親など、指定先への罰金の支払い |

このREADMEの起動・運用手順は、リポジトリ内の公開デモを対象にしています。認証は固定デモユーザーで、個人の履歴ファイルの取り込みや、指定した第三者への実際の送金は公開デモに含まれません。
...
|---|---|---|
| 自分の傾向を理解する | サンプル履歴と本人からの補足 | 日付や回数などを根拠にした、行動・挫折パターンの見立て |
