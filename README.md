...
自分で決めた罰金額も、約束の一部。「応援してもらえる喜び」と「自分で決めた約束への責任」で、目標達成を後押しします。公開デモでは、罰金はモックとして記録され、実際の課金・送金は発生しません。
...
2. **おかんの見立てを聞く。** 履歴中の具体的な出来事をもとに、つまずきやすいところや繰り返している傾向を言葉にします。
...
| 対象 | 内容 |
|---|---|
| このリポジトリの公開デモ | 公開情報データセットによる見立て、約束、写真・動画の判定、未達時の叱咤とモック罰金の記録 |
...
|---|---|---|
| 自分の傾向を理解する | 公開情報データセットと本人からの補足 | 日付や回数などを根拠にした、行動・挫折パターンの見立て |
| 約束を受け止める | 見立てと、本人が決めた目標・期限・証拠・罰金額 | その人の傾向を踏まえた、おかんからの返答 |
...
実際のアプリを、同梱の公開情報・固定デモ応答で撮影しています。判定画面もデモの例で、画像の内容をAIが評価した実績を示すものではありません。

| おかんに会う | 自分の傾向を知る |
|---|---|
| ![AIおかんのトップ画面](docs/screenshots/01-home.png) | ![公開情報を使ったおかんの見立て](docs/screenshots/02-dossier.png) |
...
利用者の状態は匿名セッションごとに分離します。ログイン・別端末への引き継ぎはありません。期限切れボタンは実課金を行わず、既存バックエンドの `penalty_transactions` に `MOCKED` として記録します。
